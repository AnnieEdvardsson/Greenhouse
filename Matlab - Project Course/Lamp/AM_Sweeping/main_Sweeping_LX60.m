...
Msg     = 'Sweeping_LX';
Info    = 'LX inside Styrobox. Intensities measured at aproximately 65cm from the lamp';

%% Here you need to add the ip-number of the current lamp 
% RX: '192.168.100.102'
% LX  '192.168.100.101'
lamp_ip = '192.168.100.101'; 
...
for i = 1:2
    ...
    for INTidx = 1:length(Intensities)
        ...
        fprintf("Sweeping nr %1.0f with LED nr %1.0f \n", INTidx, i);
        temp_intensity = zeros(size(LEDs));
        temp_intensity(LEDidx) = Intensities(INTidx);
        temp_intensity(LEDidx+1) = Intensities(INTidx);
...
save(strcat('',Msg,datestr(now,'yyyy-mm-dd-HHMM')),'intIRRmatrix','lampINTmatrix','spectra','IT','settings','Info','current_spect','LEDs','int_index','current_spect')
