...
% Calibration 2017-12-07 HL3p18
%     1) {'M1_50_CC_calib'}
%     2) {'M1_600_fib_calib'}
%     3) {'M2_600_fib_calib'}
%     4) {'STS2091_calib'}

% Calibration 2018-01-18 HL3p18 
% Calibration performed at Chalmers by Anna-Maria
%     1) {'JAZ1735'}
%     2) {'M2_600M1_fib_calib'} %With 600um fiber marked M1
%     3) {'M2_50M1_CCM_calib'}  % With 50um fiber marked M1 and CCM.
...
if strcmp(calibDate{1}, '2016-08-09')
    ...
    filename  =  'calibData_JAZ1735_2016-08-09_AM_from_Johan_Spring2017.mat';
    load(filename);
...
%% Calibrationfiles from 2017-12-07
if strcmp(calibDate{1}, '2017-12-07');
    filename = 'calibData_HL3p18_2017-12-07.mat';
    load(filename);
...
%% Calibrationfiles from 2018-01-18
if strcmp(calibDate{1}, '2018-01-18');
    filename = 'calibData_HL3p18_JAZ1735_M2_fibM1_2018-01-18.mat';
    load(filename);
    
    if strcmp(name,'JAZA1735')
        cosineCorrector = true;
        fiber_diameter  = 3900e-6;
        calibFile       = 1;
    %%%%%%%%%%%%%%% M2
    elseif strcmp(name,'MAYP11938')
        if fiberWidth == 600
            if cosineCorrector == false
                fiber_diameter = fiberWith*e-6;
                calibFile      = 2;
            elseif cosineCorrector == true
                error('No calibration found valid for this combination')
            end
        elseif fiberWidth == 50 %Fiber from M1.
            if cosineCorrector
                fiber_diameter = 3900e-6;
                calibFile      = 3; %CCM
            elseif ~cosineCorrector
                error('No calibration found valid for this combination')
            end
        end
    end
