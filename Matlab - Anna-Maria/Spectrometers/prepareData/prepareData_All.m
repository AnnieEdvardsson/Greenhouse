function DATA = prepareData_All(Spectrometers, n_spect, index)
%PREPAREDATA_ALL Summary of this function goes here
%   Detailed explanation goes here
DATA = [];
for i=1:n_spect
    current_spect=Spectrometers(i);
    ...
    [n,m] = size(current_spect.Spectra);
    for j = 1:n
...
%    specTemp = prepareData_temp(specEDZ,name);
    ...
    specIntirr = prepareData_intirr(specIrr,index(:,i)); 
    DATA(i).SERIAL = name;
    DATA(i).IRR = specIrr;
    DATA(i).intIRR = specIntirr;
