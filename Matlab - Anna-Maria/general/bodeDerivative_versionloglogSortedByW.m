function [magDer, phaseDer, ExtrVal] = bodeDerivative_versionloglogSortedByW(sys,wlim,nDeriv,fig)
...
function [SelMinPhase,SelMaxPhase,SelMinMag,SelMaxMag] = selectExtremeValues(cons,MinPhase,MaxPhase,MinMag,MaxMag)
ExtrVal= struct('MinPhase',MinPhase,'MaxPhase',MaxPhase,'MinMag',MinMag,'MaxMag',MaxMag);
ExtrValSel
for ci= 1:size(cons,2)
     sel_w = ExtrVal.(cons{ci,2});
     if ~isempty(ExtrVal.(cons{ci,1}))
         prev_w = ExtrVal.(cons{ci,1});
         for previ = length(prev_w):1 
            if prev_w(previ)<=sel_w
                prev_w_sel = prev_w(previ)
                foundPrev = true;
                break
            end
         end
     ExtrValSel.(cons{2,ci}) = [sel_w, sel_val];
     if foundPrev
        ExtrValSel.(cons{2,ci}) = [prev_w_sel, prev_val_sel];
