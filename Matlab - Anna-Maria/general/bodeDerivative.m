function [magDer, phaseDer, ExtrVal] = bodeDerivative(sys,wlim,nDeriv,fig)
...
% % false there could be only 0 or 1 row.
    ...
    magDer{1}   = [w,permute(mag,[3 2 1])];
    phaseDer{1} = [w,permute(phase,[3 2 1])];
    
    if~isempty(fig)
        cla(fig(1))
        cla(fig(2))
       semilogx(fig(1),w,magDer{1}(:,2));
       if ~ishold(fig(1))
           hold(fig(1))
       end
       semilogx(fig(2),w,phaseDer{1}(:,2));
        lgd{1} = 'bode diagram';
        if ~ishold(fig(2))
            hold(fig(2))
        end
    ...
    for derOrder = 1:nDeriv
        ...
        magDer{derOrder+1,1}  = [w_temp,magDer_temp];
        phaseDer{derOrder+1,1}= [w_temp,phaseDer_temp];
        if~isempty(fig)
            semilogx(fig(1),w_temp,magDer_temp)
            semilogx(fig(2),w_temp,phaseDer_temp)
        end
